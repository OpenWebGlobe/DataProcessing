...
int _createlayer(const std::string& sLayerName, int nLod, const std::vector<int64>& vecExtent, boost::shared_ptr<Logger> qLogger);
...
int _start(int argc, char *argv[], boost::shared_ptr<Logger> qLogger, const std::string& processpath)
{
   ...
   if (FileSystem::DirExists(sLayerPath))
   {
      ...
      else
      {
         ...
         FileSystem::rm_all(sLayerPath);
         qLogger->Info("ok.. layer deleted.");
      }
   }

   return _createlayer(sLayerName, nLod, vecExtent, qLogger);
}
