   ...
   else
   {
      for(size_t dx = offsetX; dx < (2*offsetX); dx++)
      {
         ...
         //      6 7 8
            ...
            // found no data value
            if(transparentNoData && afWin[4] <= 0.0)
            {
               foundNData = true;
            }
